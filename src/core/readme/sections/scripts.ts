...
function getScriptDescription(key: string, value: string): string {
  // Generate helpful descriptions based on script name and command
  if (key === "dev" || key === "start") return "Start development server";
  if (key === "build") return "Build for production";
  if (key === "test") return "Run test suite";
  if (key.includes("lint")) return "Lint code";
  if (key.includes("format")) return "Format code";
  if (key.includes("type")) return "Type check";
